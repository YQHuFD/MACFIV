...
MMACF <- function(X, Z){
  ...
  objective_function <- function(weights) {
    ...
    sum((X - fitted_values)^2) + 2 * sigma2 * weights %*% c(1:Q) 
  }
...
simulation_invalid <- function(n, p, s, num_simulations){
  ...
  save_path <- '/vhome/shidapeng/cd/MACFIR/invalid/s=40/'
  num_cores <- 40
  ...
  for (case in caseset){
    ...
    foreach (sim = 1:num_simulations, .combine = 'rbind', .packages = c('Rsolnp', 'MASS', 'mgcv', 'ncvreg', 'glmnet', 'splines', 'splines2')) %dopar% {
      
      mean_vector <- rep(0, p)
      indices <- 1:p
      cov_matrix <- outer(indices, indices, FUN = function(i, j) 0.5^abs(i - j))
